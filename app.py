...
from Whisper_TikTok.video_creator import VideoCreator
from Whisper_TikTok.utils import rgb_to_bgr
...
async def generate_video(
    ...
    async def get_video(video_data, args):
        with st.status("Generating video...", expanded=False) as status:
            video_creator = VideoCreator(video_data, args, logger)
...
async def main():
    ...
    with LEFT:
        ...
        tts_voice = st.selectbox(
            "TTS Voice",
            [f"{i['ShortName']} | {i['Gender']} | Tags: {i['VoiceTag']['VoicePersonalities']}" for i in await edge_tts.list_voices()], index=118, help="The voice used to generate the audio. The voice must be in the same language as the subtitles."
        )
        ...
        if st.button("Generate Video"):
            ...
            result = await generate_video(model, tts_voice, sub_position, font, font_color, font_size,
                                          url, non_english, upload_tiktok, verbose, videos, video_num, max_words)
